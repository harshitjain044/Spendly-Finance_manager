...
import { convertToMajorUnit } from "../utils/format-currency";
...
export const generateReportService = async (
  ...
  const byCategory = categories.reduce(
    (acc: any, { _id, total }: any) => {
      acc[_id] = {
        amount: convertToMajorUnit(total),
        percentage:
  ...
  const periodLabel = `${formatDateInIndia(fromDate, {
    day: "numeric",
    month: "long",
  })} - ${formatDateInIndia(toDate, {
    day: "numeric",
    month: "numeric",
    year: "numeric",
  })}`;
  ...
  return {
    period: periodLabel,
    summary: {
      income: convertToMajorUnit(totalIncome),
      expenses: convertToMajorUnit(totalExpenses),
      balance: convertToMajorUnit(availableBalance),
...
async function generateInsightsAI({
  ...
  try {
    const prompt = reportInsightPrompt({
      totalIncome: convertToMajorUnit(totalIncome),
      totalExpenses: convertToMajorUnit(totalExpenses),
      availableBalance: convertToMajorUnit(availableBalance),
      savingsRate: Number(savingsRate.toFixed(1)),
      categories,
      periodLabel,
    });
